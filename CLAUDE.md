...
- **Ревью агентом перед коммитом:** после прохождения CI-проверок, но до `git commit`, запустить отдельный Agent (subagent) для ревью изменений. Агент получает diff (`git diff --cached`), описание задачи и проверяет:
  1. **Соответствие задаче** — решение делает именно то, что было запрошено, без лишнего
  2. **Качество кода** — читаемость, типизация, отсутствие хаков, соблюдение конвенций проекта
  3. **Полнота изменений** — обновлены тесты, документация, README (если нужно), нет забытых TODO
  - Если агент нашёл проблемы — исправить до коммита. Коммитить только после успешного ревью
...
- Если правка относится к существующему коммиту (опечатка, уточнение, исправление ошибки) — вложить в тот коммит (**amend** для последнего, **fixup + rebase** для более старого), а не создавать новый коммит
