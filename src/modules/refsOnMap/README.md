...
Пока не завершена миграция legacy SVP/CUI-избранных в нативный lock (тот же gate, что в `inventoryCleanup` и `slowRefsDelete`), кнопка «На карте» скрыта, открытие viewer и удаление через trash блокируются с тостом - иначе можно было бы удалить ключи точек из legacy-списка до появления нативных f-флагов.
...
`core/refsDeletionMigrationGate.ts` (`isReferenceMassDeleteBlockedByLegacyMigration`, `getLegacyMigrationRefsDeletionBlockReason`) синхронизирует блокировку до миграции избранного с автоочисткой и slow-кнопкой: `updateButtonVisibility`, `showViewer`, `handleDeleteClick`.
