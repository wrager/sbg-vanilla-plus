...
Заменяет нативный фиксированный 32px-текст подсветки точек адаптивным размером, который читаем на любом зуме и не вращается вместе с картой. Работает для всех каналов в Layers > Text (Levels, Deployment, References, Guards) - не только References.
...
| Файл                   | Назначение                                                                                                   |
| ---------------------- | ------------------------------------------------------------------------------------------------------------ |
| `pointTextFix.ts`      | Определение модуля + `wrapFeature`/`unwrapFeature`, `wrapStyleArray`, `wrapLightRenderer`, `fontSizeForZoom` |
| `pointTextFix.test.ts` | Тесты адаптивного шрифта, обёртки renderer'a, реакции на `change`-event, race-disable, idempotency           |
