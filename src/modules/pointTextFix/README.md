...
Заменяет нативный фиксированный 32px-текст подсветки точек адаптивным размером, который читаем на любом зуме и не вращается вместе с картой. Работает для всех каналов в Layers > Text (Levels, Deployment, References, Guards) - не только References. Дополнительно поддерживает счётчик References в актуальном состоянии после изучения точки - игра не обновляет его до следующего перезапроса карты.
