...
Позволяет вращать карту круговым жестом одного пальца в режиме следования за игроком (FW). Не активируется при нативном жесте `FixedPointRotate` и подавляется во время нативного `ngrsZoom`-жеста (двойной тап + удержание).
