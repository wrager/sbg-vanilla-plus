...
- **Перетаскивание вершины схемы важнее поворота.** Пока `drawTools` тащит вершину, жест захвачен через `core/mapGestureLock.ts`, и карта не поворачивается: иначе одно движение пальца и двигало бы вершину, и крутило карту под ней. Захват появляется в середине жеста (OL шлёт `modifystart` на первом перетаскивании, уже после `touchstart`), поэтому накопленный к этому моменту поворот отбрасывается, а не применяется.
