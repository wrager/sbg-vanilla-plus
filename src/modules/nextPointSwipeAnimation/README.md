...
Решение dismiss vs return зависит от того, активен ли `improvedNextPointSwipe`:

**`improvedNextPointSwipe` активен** (нативный handler подавлен через Hammer-override):
...
**`improvedNextPointSwipe` выключен** (нативный handler жив):
...
`nearPointsSnapshot` - наша копия игрового `near_points`. Игра обновляет `near_points` в обработчике `map.on('click')` (`refs/game/script.js:536-563`): если клик пришёл по фиче слоя `points`, она перезаписывает массив на `visible.filter(isInRange(player))` - снимок точек в радиусе игрока на момент тапа. Native swipe handler на touchend читает уже этот снимок (`refs/game/script.js:737-748`), не пересчитывая на месте: если игрок успел двинуться между тапом и свайпом, native всё равно использует старый снимок. Мы поддерживаем зеркальный `nearPointsSnapshot`, чтобы предиктор dismiss/return совпадал с фактическим решением native handler-а, без расхождений из-за движения игрока.

Подписка у нас на `singleclick`, а не на игровой `click`: OL шлёт `click` сразу, а `singleclick` откладывает на 250 мс и подавляет вовсе, если пришёл второй клик (`refs/ol/ol.js:2895-2900`). Снапшот обновляется после игрового `near_points`, а на двойном тапе по точке не обновляется совсем - тогда предиктор даст `return` там, где native переключил бы точку.

Когда `improvedNextPointSwipe` выключен, наша priority logic (`pickNextInRange`) НЕ срабатывает - пользователь специально отключил «улучшенный свайп», ожидая чисто нативного поведения. Иначе модуль анимации продолжал бы тихо делать work `improvedNextPointSwipe` под видом "только анимации".

## Зачем отдельный модуль

Самостоятельная фича UI: даже без `improvedNextPointSwipe` (если пользователь его выключил) попап анимируется на свайпе и пользователь визуально видит реакцию системы. Без этого модуля свайп производит навигацию без визуального feedback - попап мгновенно меняет содержимое через `showInfo`.
