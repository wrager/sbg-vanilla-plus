...
describe('migrationUi: runFlow lock-migration-done при пустом toSend', () => {
  ...
  test('withoutKeys > 0 (легаси без стопок), alreadyApplied = 0: флаг ставится + alert о непокрытых точках', async () => {
    ...
    expect(message).toContain('legacy-n');
    // Текст содержит число и упоминание "ключей" / "keys" — часть основного
    // объяснения причины (нет ключей в инвентаре).
    expect(message).toContain('1');
    expect(message).toMatch(/ключей|keys/i);
    alertSpy.mockRestore();
  });
...
describe('migrationUi: utility функции для alert', () => {
  test('formatBriefList: до 5 элементов - все через запятую', () => {
    expect(formatBriefList([])).toBe('');
    expect(formatBriefList(['a'])).toBe('a');
    expect(formatBriefList(['a', 'b', 'c', 'd', 'e'])).toBe('a, b, c, d, e');
  });

  test('formatBriefList: 6+ элементов - "{N}: a, b, c, d, ..."', () => {
    expect(formatBriefList(['a', 'b', 'c', 'd', 'e', 'f'])).toBe('6: a, b, c, d, ...');
    expect(formatBriefList(['1', '2', '3', '4', '5', '6', '7'])).toBe('7: 1, 2, 3, 4, ...');
  });

  test('collectFailedPointTitles: уникальные titles по pointGuid из inventory-cache', () => {
    localStorage.setItem(
      'inventory-cache',
      JSON.stringify([
        { g: 's1', t: 3, l: 'p1', a: 5, ti: 'Alpha', c: [0, 0] },
        { g: 's2', t: 3, l: 'p1', a: 3, ti: 'Alpha', c: [0, 0] },
        { g: 's3', t: 3, l: 'p2', a: 1, ti: 'Beta', c: [0, 0] },
      ]),
    );
    const titles = collectFailedPointTitles([
      { itemGuid: 's1', pointGuid: 'p1' },
      { itemGuid: 's2', pointGuid: 'p1' },
      { itemGuid: 's3', pointGuid: 'p2' },
    ]);
    // Уникальные по pointGuid, отсортированные.
    expect(titles).toEqual(['Alpha', 'Beta']);
  });

  test('collectFailedPointTitles: пропускает стопки, для которых точка отсутствует в кэше', () => {
    localStorage.setItem(
      'inventory-cache',
      JSON.stringify([{ g: 's1', t: 3, l: 'p1', a: 5, ti: 'Alpha', c: [0, 0] }]),
    );
    const titles = collectFailedPointTitles([
      { itemGuid: 's1', pointGuid: 'p1' },
      { itemGuid: 's-missing', pointGuid: 'p-missing' },
    ]);
...
describe('migrationUi: e2e успешная миграция в locked выставляет lock-migration-done', () => {
  ...
  test('partial: networkFailed на одной стопке - modal alert вместо toast, флаг НЕ ставится', async () => {
    await seedFavorites([{ guid: 'p1', cooldown: null }]);
    await loadFavorites();
    // Две стопки одной точки, разные guid - чтобы первая прошла, вторая упала
    // в network. Title на стопке - используется в alertPartial для списка
    // затронутых точек.
    setInventoryCache([
      { g: 's1', t: 3, l: 'p1', a: 5, ti: 'TestPoint', c: [0, 0], f: 0 },
      { g: 's2', t: 3, l: 'p1', a: 3, ti: 'TestPoint', c: [0, 0], f: 0 },
    ]);
    expect(isLockMigrationDone()).toBe(false);

    // Первый запрос success, дальше ВСЕ networkFailed (исходник + 3 retry-фазы
    // с backoff [5s, 15s, 45s]). Используем fake timers, чтобы ускорить.
    jest.useFakeTimers();
    ...
    // Прокручиваем все таймеры (request delay + 3 retry backoff'а).
    for (let i = 0; i < 100; i++) {
    ...
    // Partial - флаг не ставится.
    expect(isLockMigrationDone()).toBe(false);

    // Modal alert вместо toast - на мобильном устройстве toast мог бы пройти
    // незамеченным, partial требует явного сигнала "повтори через пару минут".
    expect(alertSpy).toHaveBeenCalled();
    const message = String(alertSpy.mock.calls[alertSpy.mock.calls.length - 1][0]);
    // Сообщение содержит число успешных и общее, упоминание "минут" или
    // эквивалент, и title затронутой точки в списке.
    expect(message).toContain('1');
    expect(message).toContain('2');
    expect(message).toMatch(/минут|minute/i);
    expect(message).toContain('TestPoint');
    alertSpy.mockRestore();
  });
});
