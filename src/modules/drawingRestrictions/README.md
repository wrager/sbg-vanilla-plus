...
- **Режим «звезда»** — одна точка фиксируется как центр звезды. Когда открыт попап самого центра, список рисования не фильтруется (все линии из центра — часть звезды). Когда открыт попап любой другой точки, в списке остаётся только центр звезды — нельзя случайно нарисовать «не звёздную» линию. Центр **дополнительно** обводится жёлтым кольцом на карте (стандартный вид точки сохраняется — иконка, цвет команды). Управление:
  ...
  - **Locked-точку нельзя назначить центром звезды.** Нативный замочек защищает ключи от расходования на линии, поэтому из такой точки невозможно было бы нарисовать ни одной линии. При клике «назначить» в попапе locked-точки показывается предупреждающий toast; если центр уже был назначен на не-locked точку, он снимается (намерение переключиться выражено, а оставлять прежний центр после блок-сообщения было бы неожиданно).
