...
Модуль добавляет режим рисования звезды (множество линий, сходящихся в одну зафиксированную точку-центр) и фильтр дальности для списка целей рисования. Фильтрация делается на клиенте: сам запрос к серверу не модифицируется — перехватывается только ответ `GET /api/draw`.
...
- **Режим «звезда»** — одна точка фиксируется как центр звезды. Когда открыт попап самого центра, список рисования не фильтруется (все линии из центра — часть звезды). Когда открыт попап любой другой точки, в списке остаётся только центр звезды — нельзя случайно нарисовать «не звёздную» линию. Центр **дополнительно** обводится жёлтым кольцом на карте (стандартный вид точки сохраняется — иконка, цвет команды).
  ...
  Locked-точку центром звезды не назначить. Все её ключи защищены замочком и не расходуются на линии. Клик «назначить» в попапе locked-точки показывает предупреждение; ранее назначенный центр снимается.
...
- `starCenter*` — компоненты режима звезды: `starCenter.ts` (хранилище + событие), `starCenterButton.ts` (кнопка в попапе с проверкой lock-флага через `buildLockedPointGuids` перед назначением), `starCenterClearControl.ts` (OL-control на карте), `starCenterHighlight.ts` (overlay-слой с жёлтым кольцом), `starCenterIcon.ts` (SVG иконок), `starCenterToasts.ts` (toast-функции «центр назначен» / «центр снят» / «нельзя назначить locked-точку центром»).
