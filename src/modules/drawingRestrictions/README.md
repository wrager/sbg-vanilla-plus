...
- **Защита точек с замочком** — три режима. Источник истины — нативный lock-флаг SBG 0.6.1 (бит `0b10` поля `f` стопки ключей в `inventory-cache`). Игрок ставит/снимает замочек штатной кнопкой игры или массово через модуль `favoritesMigration` из старого SVP/CUI-списка.
  - _Без защиты_ — ключи защищённых точек в списке рисования показываются наравне с остальными.
  - _Защищать только последний ключ_ — из списка убирается только защищённая цель, где у игрока остался ровно 1 ключ (чтобы не потратить последний при рисовании).
...
| Только последние ключи      | «Скрыт(ы) последн(ий/ие) N ключ(/а/ей) от защищённых точек»        |
