...
Модуль — единая точка контроля для всех путей синхронизации (см. ниже раздел «Owner-тумблер»). Отключение модуля останавливает sync от любого источника, не только от discover-перехвата.
...
Симптом проявляется для нативного текста подсветки точки: native `FeatureStyles.LIGHT` renderer читает `feature.get('highlight')['7']` через closure и не пересчитывается при изменении значения.
...
Sync ходит в игровой OL-слой и в `inventory-cache`, поэтому после смены версии игры его промис может отклониться. Отказ пишется в консоль один раз за включение модуля и дальше игнорируется: игре он не виден - её `fetch` уже отдан, - но без обработки стал бы unhandled rejection. Перехват при этом остаётся, следующий discover снова запускает sync.
