...
Обновление счётчика ключей на подписи точки сразу после изучения - в отдельном модуле `fixRedrawRefsOnDiscover` (категория багфиксов), независимо от `improvedPointText`. Когда оба активны, наш wrap читает обновлённое значение из того же `prop.highlight` массива, что мутирует fix-модуль.
