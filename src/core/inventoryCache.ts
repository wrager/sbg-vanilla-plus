 ...
 * хранения; в UI игрок видит точку, и lock-семантика пользователю «заблокировать
 * ключи этой точки от удаления». Поэтому агрегируем per-point: одна locked
 * стопка - вся точка удерживается от удаления.
 *
 ...
 * «удерживается от удаления». Для логики удержания от удаления есть
...
/**
 * Возвращает GUID'ы точек, удерживаемых от удаления: хотя бы одна стопка ключей
 ...
 * стопка с замочком или звёздочкой - вся точка удерживается. Игрок видит точку, не стопку.
 */
...
/**
 * Можно ли полагаться на поле `f` стопок для определения удержания ключей. Поле
 * появилось в SBG 0.6.1; на 0.6.0 его нет ни у одной стопки, и любое удержание
 ...
 * точка с замочком или звёздочкой по своей mix-стопке могла быть удалена.
