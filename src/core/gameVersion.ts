// Прошлая версия остаётся в списке, пока точки сцепления совместимы. Заголовок
// описывает версию сервера, а не загруженного клиента, и на случай их
// расхождения (незавершённая раскатка, кеширование клиента браузером) прошлая
// версия считается совместимой. Иначе игрок получил бы блокирующий confirm на
// каждой загрузке: выбор намеренно не запоминается (см. gameVersionPrompt), так
// что на телефоне это модалка при каждом открытии игры. Цена лишней записи -
// одна устаревающая строка, цена её снятия - диалог у части игроков.
// Порядок от новой версии к старой: список идёт в описание userscript и в текст
// confirm, где первой читается версия, под которую скрипт делается сейчас.
...
// и/или runtime-детекцией native), часть удалены физически (keyCountOnPoints,
// repairAtFullCharge, ngrsZoom), сет очищен.
