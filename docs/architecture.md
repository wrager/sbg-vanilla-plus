...
**Версия SBG** — `src/core/gameVersion.ts`: `SBG_COMPATIBLE_VERSIONS` (поддерживается v0.6.1). Проверяет заголовок `x-sbg-version` из любого `/api/*` ответа через perf-патч `window.fetch` в document-start. Сеты `DEPRECATED_MODULES_NATIVE` и `DEPRECATED_MODULES_CONFLICTED` пусты: текущий состав модулей либо опирается на нативные API 0.6.1 как источник истины (`inventoryCleanup` использует lock-флаг `inventory-cache[i].f & 0b10` как защиту ключей и принудительно отключает нативный «Сборщик мусора» через `nativeGarbageGuard`; `drawingRestrictions` использует тот же lock-флаг как источник списка защищённых точек для фильтра рисования; `favoritesMigration` переносит legacy SVP/CUI-список в нативные звёздочки/замочки через POST `/api/marks`; `improvedNextPointSwipe` подменяет нативный горизонтальный свайп через runtime-override `Hammer.Manager.prototype.emit` и реализует свою приоритетную навигацию в радиусе взаимодействия; `swipeToClosePopup` живёт на общей инфраструктуре `core/popupSwipe`), либо мирно сосуществует с новыми нативными жестами через state-machine подавления (`singleFingerRotation` во время нативного `DblClickDragZoom`). Инфраструктура подавления через `DEPRECATED_MODULES_NATIVE` / `DEPRECATED_MODULES_CONFLICTED` оставлена для будущих версий игры.
