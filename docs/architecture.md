...
**Плавное движение маркера (smoothPlayerMarker)** — map-модуль, сглаживающий рендер маркера игрока между фиксами геолокации. Геометрия `player_feature` остаётся ровно на последнем реальном фиксе: игра отправляет её на сервер в каждом действии (`position:` в discover/deploy/attack/draw, `pos` в websocket) и по ней считает `isInRange`, поэтому интерполяция в неё — подмена GPS и модификация запросов. Вместо этого интерполируются геометрия рендера иконки (`Style.setGeometry` на `player_styles[0]`, refs/ol/ol.js:6884) и центры кругов радиуса действия и взрыва (`player_styles[1..3]`, только центр — радиусы остаются игровыми, на них держится анимация взрыва). После каждого применённого кадра — `feature.changed()`, как это делает сама игра на 45 Гц для компаса. Триггер — игровое событие `playermove`: игра диспатчит его внутри `movePlayer` после записи координат, поэтому цель читается прямо из фичи; повторные вызовы `movePlayer` с той же координатой распознаются как перерисовка и не сбивают измеренный интервал между фиксами. Длительность анимации равна этому интервалу, зажатому в 200-2000 мс; скачок больше 100 м применяется мгновенно. Плавный follow сделан обёрткой `view.setCenter`: игровой вызов с координатой фикса, пришедший в окне фикса (окно закрывается микротаском, то есть после синхронного хвоста `movePlayer`), перенаправляется на текущую интерполированную точку, дальше центр ведут кадры анимации; `view.getInteracting()` отпускает центр на время жеста пользователя. `view.animate` не используется: во время OL-анимации подавлен `moveend` (refs/ol/ol.js:8169), а на нём висит `requestEntities()` игры, и точки перестали бы догружаться. Подробности и отвергнутые варианты — в `src/modules/smoothPlayerMarker/README.md`.
...
| `smoothPlayerMarker`                             | Обёртка `view.setCenter` на экземпляре view плюс мутация игровых `player_styles`  | Полностью снимается на disable: `setCenter` восстанавливается из сохранённой ссылки, иконке возвращается геометрия фичи (`setGeometry(undefined)`), центры кругов встают на реальный фикс, кадры анимации отменяются                                                                                                                                                                        |

Полное снятие — только перезагрузкой страницы для `refsLayerSync`, `xpPopups`, `drawingRestrictions.drawFilter` и серверной части `nativeGarbageGuard`. Остальные override (`improvedNextPointSwipe`, `smoothPlayerMarker`, DOM-часть `nativeGarbageGuard`) снимаются полностью на disable. Это сознательное архитектурное решение в пользу простоты install-логики (`installGeneration` race protection остаётся, fully-clean uninstall обязателен только там, где модуль предполагает многократный re-enable за сессию).

### Ошибка нашей логики внутри override

Через override проходит весь поток жестов и сети игры, поэтому исключение из нашего кода не должно выходить в игровой. Правило одинаковое для всех перехватов: собственная логика идёт в `try`, вызов оригинала — вне него, в `catch` пишется `console.error` с префиксом `[SVP <moduleId>]` и разбор пропускается. Перехват при этом не снимается: он остаётся в цепочке и продолжает работать на следующих вызовах, потому что ошибка обычно относится к конкретным данным, а не к совместимости навсегда. Снятие оправдано только там, где перехват украшает уже работающее поведение игры и его снятие возвращает игре собственный путь — так сделан `compactToasts`, отдающий игре её показ сообщений. Там, где на перехвате держится координация модулей или состояние enable, снятие ломает их: снятый Hammer-override перестал бы подавлять нативный handler, который `nextPointSwipeAnimation` считает подавленным.

Запись делается один раз за включение модуля (флаг в модуле, сбрасывается в `enable`): точки перехвата вызываются на каждом жесте и каждом запросе, и повтор вытеснил бы полезные строки из `core/errorLog`. `try` ставится там, где действительно есть источник — выражение, способное бросить на данных игры (OL-фичи, DOM игры, данные ответа сервера), а не вокруг проверок типов: разбор аргументов запроса и чтение попапа через `querySelector` с константным селектором бросить не могут и в try не заворачиваются.

Отдельный случай — логика, идущая ДО вызова оригинала. Такая есть только в `sbgFlavor`, и она изолируется всегда: её отказ стоит игре самого запроса, а не разбора результата. Остальные перехваты зовут оригинал первой строкой и работают с уже отданным запросом, поэтому их отказ ограничен нашей частью.
