...
**Версия SBG** — `src/core/gameVersion.ts`: `SBG_COMPATIBLE_VERSIONS` (поддерживается v0.6.1). Проверяет заголовок `x-sbg-version` из любого `/api/*` ответа через perf-патч `window.fetch` в document-start. Сеты `DEPRECATED_MODULES_NATIVE` и `DEPRECATED_MODULES_CONFLICTED` сейчас пусты — модули, чей use case был перекрыт нативно в 0.6.1, либо адаптированы (lock-aware deletion в `inventoryCleanup` с блокировкой при pending миграции и принудительным отключением нативного сборщика мусора через `nativeGarbageGuard`, state-machine подавления `singleFingerRotation` во время нативного `DblClickDragZoom`-жеста, drawingRestrictions переключён с локального SVP/CUI-списка на нативный lock-флаг (бит `0b10` поля `f` стопки), legacy-модуль `favoritedPoints` удалён, миграция в нативные звёздочки/замочки выделена в отдельный модуль `favoritesMigration`, `nextPointNavigation` удалён в пользу `improvedNextPointSwipe` (свайп-навигация через runtime-override `Hammer.Manager.prototype.emit` с приоритетной навигацией в радиусе взаимодействия), `swipeToClosePopup` переехал на общий `core/popupSwipe`), либо удалены физически (`repairAtFullCharge`, `ngrsZoom`, `keyCountOnPoints`). Инфраструктура подавления оставлена для будущих версий игры.
