...
**Тосты** — `src/core/toast.ts` и `src/core/toastify.ts`: `showToast(message, { duration, type, position, container })` рисует уведомление SVP через Toastify-JS 1.12.0, который игра подключает обычным `<script>` (`refs/game/index.html:35`) и через который показывает все свои сообщения. Своей раскладки у SVP больше нет: одновременные тосты Toastify разводит сам (`reposition`, `refs/toastify/toastify.js:117`), а до переезда все наши сообщения рисовались в одной фиксированной точке и накладывались друг на друга. Типов сообщения ровно два — те же классы, что у игры: `interaction-toast` и `error-toast`; свой цвет выбивался бы из игрового интерфейса. Если Toastify недоступен, `showToast` уходит на прежнюю собственную реализацию (`.svp-toast` из `core/toast.css`) — путь для тестов и нештатной загрузки SVP на чужой странице: сама игра без Toastify не стартует (`refs/game/script.js:16-27`). `core/toastify.ts` держит типы API библиотеки, `getToastifyFactory()` (фабрика или `null`) и `isErrorToast()` — проверку класса по токену, потому что `className` допускает несколько классов через пробел. Потребители типов: `core/toast.ts` и `compactToasts`.

**Сборка тостов (compactToasts)** — ui-модуль, патчащий `Toastify.prototype.showToast` (единственная точка, через которую проходят все уведомления игры) и собирающий одновременные тосты уровня экрана в один блок в левом верхнем углу: строки в порядке первого появления, повтор увеличивает счётчик, не больше пяти строк, класс блока становится `error-toast`, если хотя бы одно сообщение было отказом. Прежний узел снимается напрямую (`clearTimeout(timeOutValue)` + `remove()`), потому что штатный `removeElement` откладывает удаление на 400 мс и всё это время на экране висели бы два блока. Инстансы не создаются, меняется входящий — учёт `popup_toasts` внутри игры остаётся согласованным. Граница проходит по контейнеру: тосты с `selector` (попап точки, инвентарь, профиль, уведомления) отдаются оригиналу нетронутыми, потому что тост с добычей игра создаёт пустым и дописывает содержимое уже после показа (`refs/game/script.js:830-845`) — подмена узла стёрла бы игроку список добычи. Длинный трёхстрочный тост про новые регионы сворачивается в строку вида `Регионы: +3 (1.4 км²)`: `regionsLine.ts` опознаёт его по шаблону `popups.new-regions` из локализации игры (`i18next.getResource`), берёт подпись из игрового ключа `info.regions` и переиспользует уже отформатированную игрой площадь, поэтому собственных переводов не содержит. Не опознали — текст показывается целиком, без обрезки. Подробности и отвергнутые подходы — в `src/modules/compactToasts/README.md`.
