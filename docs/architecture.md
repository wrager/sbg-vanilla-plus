...
**Подписи на точках (pointTextFix)** — гибрид runtime-обёртки нативного LIGHT-renderer и собственного overlay-слоя для канала References. Wrap-часть: на enable `feature.setStyle` подменяется per-feature, новые style array проходят через `wrapStyleArray` со заменой нативного renderer на обёртку с Symbol-маркером; подписка на `feature 'change'` ловит in-place мутации `style[1]` в showInfo/attack response (refs/game/script.js около 2789-2796). Обёртка на каждом render call создаёт Proxy вокруг `state.context`: при установке `ctx.font` подменяет любое `Npx` на адаптивный размер `clamp(10, zoom-3, 16) * pixelRatio` (множитель повторяет поведение OL `Text` style: `textScale = pixelRatio * scale`); при `ctx.fillText`/`ctx.strokeText` компенсирует поворот канваса через `save -> translate(x,y) -> rotate(-state.rotation) -> translate(-x,-y) -> orig -> restore`. Цвета (`fillStyle`, `strokeStyle`) и геометрия (кольца, сектора, прогресс-бары, числа Levels/Deployment/Guards) идут нативно. Чтобы нативный текст канала References не дублировался с нашим overlay, обёртка перед вызовом original в `try/finally` подменяет `feature.get('highlight')[7]` на `undefined` и восстанавливает после: native LIGHT в case 7 имеет ранний `if (typeof value === 'undefined') continue` (refs/game/script.js около 304), поэтому native refs не рисуется ни в одном слоте, а другие читатели `prop.highlight` (showInfo, attack, drawEntities) видят исходные данные. Overlay-часть: отдельный `ol.layer.Vector` (имя `svp-point-text-fix`, `zIndex: 5`), источник чисел - `inventory-cache` через `buildRefCounts` (ключи по `t === 3`, агрегация по `l`, сумма `a`); ре-рендер по `pointsSource.on('change')` debounced 100мс, `view.on('change:resolution')` без debounce, `MutationObserver` на `#self-info__inv` (счётчик инвентаря в шапке игры). `installGeneration` counter защищает от race-disable во время `await getOlMap()`.
