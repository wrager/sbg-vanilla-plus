...
**Игровые настройки** — `src/core/gameSettings.ts`: `readGameSetting(key)` читает `localStorage['settings']` и при недоступном значении отдаёт дефолт игры. Дефолты дублируются только для полей, которые реально читает SVP (`lang`, `theme`): полный объект из 20 полей пришлось бы сверять с игрой при каждом её обновлении. Отсутствие ключа — штатное состояние: SBG 0.7.0 убрал материализацию `settings` при первом запуске (`refs/game/script.js:3641`), игра читает значения через `getLocalStorageDefault` (`refs/game/script.js:3674`) и пишет ключ только при первом изменении настройки. До появления модуля ключ разбирался в каждом потребителе отдельно и с разными фолбэками, из-за чего его отсутствие означало «английский язык» в `l10n` и «светлая тема» в `favoritesStore`. Потребители: `l10n.getGameLocale` (поле `lang`), `favoritesStore` (поле `theme` для сида CUI-конфига).
